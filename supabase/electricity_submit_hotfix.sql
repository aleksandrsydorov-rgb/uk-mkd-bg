-- =============================================================================
-- AMADEUS 11 — electricity submit hotfix
-- =============================================================================
-- Adds building_settings.electricity_mode, source/idempotency on meter_readings,
-- submit_electricity_reading RPC, and tightens INSERT so clients cannot bypass
-- mode/ownership checks. Does not alter water_* objects.
--
...
-- ---------------------------------------------------------------------------
-- building_settings.electricity_mode
-- ---------------------------------------------------------------------------

alter table public.building_settings
  add column if not exists electricity_mode text;

update public.building_settings
   set electricity_mode = 'owner_and_staff'
 where id = 1
   and (electricity_mode is null or btrim(electricity_mode) = '');

alter table public.building_settings
  alter column electricity_mode set default 'owner_and_staff';

do $$
begin
  alter table public.building_settings
    add constraint building_settings_electricity_mode_chk
    check (electricity_mode in ('owner_and_staff', 'staff_only', 'disabled'));
exception
...
-- ---------------------------------------------------------------------------
-- meter_readings source + idempotency
-- ---------------------------------------------------------------------------

alter table public.meter_readings
  add column if not exists submitted_source text;
...
begin
  alter table public.meter_readings
    add constraint meter_readings_submitted_source_chk
    check (submitted_source is null or submitted_source in ('owner', 'staff'));
exception
  when duplicate_object then null;
end
$$;

create unique index if not exists meter_readings_electricity_idempotency_idx
  on public.meter_readings (idempotency_key, meter_type)
  where idempotency_key is not null;

create index if not exists meter_readings_property_type_date_idx
  on public.meter_readings (property_id, meter_type, reading_date desc, created_at desc);

-- ---------------------------------------------------------------------------
-- RLS: select stays; insert only via RPC; no owner update/delete
-- ---------------------------------------------------------------------------

alter table public.meter_readings enable row level security;

drop policy if exists meter_readings_staff_insert on public.meter_readings;
drop policy if exists meter_readings_owner_insert on public.meter_readings;
drop policy if exists meter_readings_staff_update on public.meter_readings;
drop policy if exists meter_readings_owner_update on public.meter_readings;
...
drop policy if exists meter_readings_owner_staff_select on public.meter_readings;
