-- =============================================================================
-- AMADEUS 11 — electricity staff authorization patch
-- =============================================================================
-- Apply after electricity_submit_hotfix.sql on an already-deployed database.
-- Replaces submit_electricity_reading only. No table/RLS changes.
--
-- Keeps has_staff_role() strict rule: staff.active IS TRUE.
-- Does NOT treat active IS NULL as authorized.
-- Does NOT query staff with active IS NOT FALSE.
-- Fixes mode branching: staff_only allows authorized staff without owns_property.
--
-- DATA FIX is NOT included: this workspace did not query live staff rows,
-- so no email is filled in. Run electricity_staff_null_active_diag.sql (or the
-- trailing SELECT in electricity_submit_verify.sql). If the current test admin
-- appears there with active IS NULL, apply only that row:
--
--   UPDATE public.staff
--   SET active = true
--   WHERE lower(email) = lower('<CURRENT_ADMIN_EMAIL>')
--     AND active IS NULL;
--
-- Replace <CURRENT_ADMIN_EMAIL> with the exact email from the diagnostic result.
